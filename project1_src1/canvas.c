...
void myKeyHandler(unsigned char ch, int x, int y) {
    switch(ch) {
        ...
        case 'i':
            if(disp_mode == DM_CONE_TRI_CALC){
                if(r >= .2){
                    ...
                    printf("Radius already at min value\n");
            }
            break;
        case 'I':
            if(disp_mode == DM_CONE_TRI_CALC){
        ...
        case 'o':
            if(disp_mode == DM_CONE_TRI_CALC){
                if(h >= .2){
                    ...
                    printf("Height already at min value\n");
            }
            break;
        case 'O':
            if(disp_mode == DM_CONE_TRI_CALC){
        ...
        case 'p':
            if(disp_mode == DM_CONE_TRI_CALC){
                if(n >= 4){
                    printf("Base triangles decreased by 1\n");
                    n--;
                }
                else
                    printf("Base triangles already at min value\n");
            }
            break;
        case 'P':
            if(disp_mode == DM_CONE_TRI_CALC){
                n++;
                printf("Base triangles increased by 1\n");
            }
            break;
        case 'v':
            if(disp_mode == DM_VRML){
                vr_object = ++vr_object % VR_MAX;
                print_vrml_object();
            }else
                return;
            break;
